...
from hotel import get_hotel  # Areej
from datetime import date, datetime
from google_places import get_place  # Areej
from locations import get_location  # Areej
...
def result():
    ...
    checked_cats.append('amusement park') if 'amusement_park' in request.form else None  # check if nightlife are selected
    checked_cats.append('tourist attraction') if 'tourist_attraction' in request.form else None  # check if shopping are selected
    checked_cats.append('shopping mall') if 'shopping_mall' in request.form else None  # check if shopping are selected
    checked_cats.append('aquarium') if 'aquarium' in request.form else None

    days_diff = datetime.strptime(str(endday).strip(), '%Y-%m-%d') - datetime.strptime(str(startday).strip(), '%Y-%m-%d')
    days = days_diff.days
    print('cats', checked_cats)
    ...
    print(Airline_start_code, Airline_end_code, startday, travellers)
    ...
    hotel_data = get_hotel(Airline_end)  # get hotel data from here
    days_activities = get_place(Airline_end, days, checked_cats, 2)  # get days activities
